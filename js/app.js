...
*/
    ...
    function initSliders() {
        ...
        if (document.querySelector(".how__slider")) new Swiper(".how__slider", {
            observer: true,
            observeParents: true,
            slidesPerView: 0,
            spaceBetween: 0,
    ...
    const tableBlock = document.querySelector(".table__box");
    const arrowButton = document.querySelector(".table__arrow");
    if (arrowButton) {
        const scrollStep = 300;
        function updateArrowVisibility() {
            const maxScrollLeft = tableBlock.scrollWidth - tableBlock.clientWidth;
            if (maxScrollLeft > 0) if (tableBlock.scrollLeft >= maxScrollLeft - 1) arrowButton.classList.add("hidden"); else arrowButton.classList.remove("hidden"); else arrowButton.classList.add("hidden");
        }
        ...
        tableBlock.addEventListener("scroll", updateArrowVisibility);
    ...
    const svgAll = document.querySelectorAll(".roadmap__svg");
    if (svgAll.length) svgAll.forEach((svg => {
        const mainLine = svg.querySelector(".main-line");
        const leftPaths = svg.querySelectorAll(".left-path");
        const rightPaths = svg.querySelectorAll(".right-path");
        if (!mainLine) return;
        const branchPositions = [ .17, .33, .49, .55, .7, .85 ];
        ...
        const mainLength = preparePath(mainLine);
        leftPaths.forEach(preparePath);
        rightPaths.forEach(preparePath);
        ...
        function animateLines() {
            ...
            tl.to(mainLine.style, {
                strokeDashoffset: 0,
                duration: 3,
                onUpdate() {
                    const drawnLength = mainLength - parseFloat(mainLine.style.strokeDashoffset);
                    branchPositions.forEach(((pos, i) => {
                        const y = mainLength * pos;
                        if (i >= 3) {
                            const path = leftPaths[i - 3];
                            if (path && !path.dataset.animated && drawnLength >= y) {
                                path.dataset.animated = "true";
                                gsapWithCSS.to(path.style, {
                                    strokeDashoffset: 0,
                                    duration: 1
                                });
                            }
                        } else {
                            const path = rightPaths[i];
                            if (path && !path.dataset.animated && drawnLength >= y) {
                                path.dataset.animated = "true";
                                gsapWithCSS.to(path.style, {
                                    strokeDashoffset: 0,
    ...
    const tokenomicsImages = document.querySelectorAll('[class^="tokenomics__image-"]');
    if (tokenomicsImages.length) tokenomicsImages.forEach(((el, index) => {
